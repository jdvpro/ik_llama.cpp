...
class LazyTorchTensor(gguf.LazyBase):
    ...
    _dtype_byteswap_map: dict[torch.dtype, type] = {
        ...
        **(({torch.uint64: np.uint64} if hasattr(torch, 'uint64') else {})),
        torch.int32: np.int32,
        **(({torch.uint32: np.uint32} if hasattr(torch, 'uint32') else {})),
        torch.int16: np.int16,
        **(({torch.uint16: np.uint16} if hasattr(torch, 'uint16') else {})),
        torch.int8: np.int8,
        torch.uint8: np.uint8,
        torch.bool: np.uint8,
