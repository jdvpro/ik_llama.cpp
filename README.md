...
Задача. Сделать возможность конвертировать и квантизировать модели с MTP весами. Если это проще сделать отдельным python скриптом,
чем модифицировать существующие скрипты ik_llama.cpp (текущий проект) - меня это тоже устроит даже больше чем стандартный подход.
Модель с MTP вот: @/home/agent/models/Qwen3.6-35B-A3B-iq4_xs.gguf,
То есть оттуда можно достать названия и все остальное.

Для начала просто проверь насколько это проблематично, то есть можешь сделать дамп MTP тензоров (если это необходимо на данном этапе) и т.д.

Там же в папке @/home/agent/models/ есть папка Qwen - это hf модели


Очень важная деталь. qwen3next - это базовая архитектура для qwen35moe, фактически оно одно и то же.  Но заметь модели есть и такие и такие.
Есть Qwen3-Next-80B-A3B-Instruct-iq4_xs.gguf и есть Qwen3.6-35B-A3B-iq4_xs.gguf - Это две разные модели, которые разных классов требуют,
но внутри это одно и то же. И вот мне важно, чтобы конвертер поддерживал их обе.

В наличии у меня модели Qwen3-Next-80B-A3B-Instruct с MTP тензорами нет. И не могу найти её на hf. Это главная мотивация почему мы сейчас занимаемся тем чем занимаемся.
