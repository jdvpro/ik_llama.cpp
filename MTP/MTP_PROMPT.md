...
**For Agent:**  
При загрузке модели выведи в лог все тензоры, имя которых содержит "mtp". Для каждого — имя, shape, dtype. В конце — общее количество загруженных тензоров (всех, не только mtp).
...
**For Agent:**  
Сделай тестовый режим (флаг или ifdef), который при старте:

1. Берёт embedding BOS-токена как вход
2. Прогоняет его через одну MTP-голову (голова 0)
3. Выводит в лог:
    - shape выхода (logits)
    - top-5 токенов с их logit-значениями (decode в текст)
    - min, max, mean logits
    - есть ли NaN или Inf (да/нет)
4. Прогоняет тот же вход второй раз и сообщает: результат идентичен первому? да/нет
5. Завершает работу (exit)
...
**For Agent:**  
При аллокации KV cache для MTP-слоёв выведи в лог: layer_id, n_ctx, n_heads, head_dim, total_bytes.
Затем прогони prompt "Hello world" (только prompt processing, без генерации). Выведи в лог: сколько позиций заполнено в KV cache основной модели и сколько в KV cache MTP.
Потом прогони prompt из 100 токенов (любой текст). Те же числа.
Не должно быть крашей или ассертов.
...
**For Agent:**  
Режим генерации: после каждого шага основной модели прогони hidden state через MTP-головы и залогируй draft-токены. Но НЕ используй их для ускорения — основная модель работает как обычно.
Формат лога на каждом шаге:
[step N] main: "слово" | mtp0_draft: "слово" | mtp0_match: yes/no
mtp0_match = совпал ли draft-токен MTP с тем, что основная модель выдала на СЛЕДУЮЩЕМ шаге.

В конце генерации выведи:
- Полный сгенерированный текст (основная модель)
- Acceptance rate = процент "yes" в mtp0_match

Параметры: temperature=0, greedy decoding. Prompt: "Explain what is machine learning in simple terms". Генерация 80 токенов.
...
**For Agent:**  
Прогони 5 промптов (дам список) в том же режиме что этап 5. Для каждого выведи: сгенерированный текст и acceptance rate.
